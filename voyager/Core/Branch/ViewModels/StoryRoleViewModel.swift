...
class StoryRoleModel: ObservableObject {
    ...
    func generateRoleDescription(storyId: Int64,roleId:Int64,userId:Int64,sampleDesc: String) async -> (Common_CharacterDetail?,Error?){
    ...
    func generateRolePrompt(storyId: Int64,roleId:Int64,userId:Int64,samplePrompt: String) async -> (String?,Error?){
        let (prompt,err) = await APIClient.shared.generateRolePromppt(storyId: storyId, roleId: roleId, userId: userId, samplePromppt: samplePrompt)
        if err != nil{
    ...
    func updateRolePrompt(userId: Int64,roleId:Int64,prompt: String)async -> Error?{
        let err = await APIClient.shared.updateStoryRolePrompt(userId: userId, roleId: roleId, prompt: prompt)
        if err != nil{
    ...
    func updateStoryRoleBackground(userId: Int64,roleId: Int64,backgroundAvatar: String) async -> Error?{
